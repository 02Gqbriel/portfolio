...
>
	...
	<button class="w-5 h-5 overflow-hidden" on:click={toggleDarkmode}>
		<svg
			...
			viewBox="0 0 20 20"
			fill="currentColor"
			class="w-5 h-5 transition-transform {!pastHeader && 'fill-white'} {dark &&
				'translate-y-full'}"
		>
			<path
				fill-rule="evenodd"
...
<nav class="md:hidden block">
	...
	<div
		on:click={toggleMenu}
		class="fixed h-screen w-screen z-40 blur backdrop-blur-sm top-0 {menuOpen
			? 'right-0'
			: '-right-full'}"
	/>
	...
	>
		<div class="grow flex flex-col space-y-4 p-2 mt-3">
			...
			>
				...
				<span class="grow"> contact me </span>
			</a>
		</div>
		...
		>
			<svg
				...
				viewBox="0 0 20 20"
				fill="currentColor"
				class="w-8 h-8 transition-transform {dark && 'translate-y-full'}"
			>
				<path
					fill-rule="evenodd"
					d="M7.455 2.004a.75.75 0 01.26.77 7 7 0 009.958 7.967.75.75 0 011.067.853A8.5 8.5 0 116.647 1.921a.75.75 0 01.808.083z"
		...
		<div class="flex justify-evenly p-1 items-center">
			<svg
				xmlns="http://www.w3.org/2000/svg"
				viewBox="0 0 20 20"
				fill="currentColor"
